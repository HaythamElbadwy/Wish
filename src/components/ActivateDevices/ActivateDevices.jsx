...
export default function ActivateDevices() {
  ...
  const handlePayment = async (e) => {
    ...
    // Payment Intent
    const response = await fetch('https://wish-omega-blush.vercel.app/subscription/pay', {
      method: 'POST',
      headers: {
  ...
  const subscription = async () => {
    try {
      const response = await fetch(`https://wish-omega-blush.vercel.app/subscription/start`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'authorization': `wisOZ0${localStorage.getItem('authToken')}`
